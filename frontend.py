...
# --- 1. CARICAMENTO DATI (CACHED) ---
# Questa funzione gira una volta sola e si ricorda i dati per non rallentare
@st.cache_resource
...
# --- 2. CONFIGURAZIONE AI ---
# Cerca la chiave nei Secrets di Streamlit (Online) o nel file .env (Locale)
api_key = st.secrets.get("GOOGLE_API_KEY")
if not api_key:
    # Fallback per locale se non usi secrets.toml ma .env
...
if api_key:
    ...
    st.error("⚠️ Chiave API mancante! Configurala nei Secrets di Streamlit.")
...
if prompt := st.chat_input("Come ti senti?"):
    ...
    # FASE 2: CLINICA (Se non è emergenza)
    if not is_emergency and api_key:
        try:
